...
from ia.memoria_jugador import BattleTracker, actualizar_perfil
...
class PokemonGUI:
    ...
    def __init__(self, screen, ai_level=1, battle_type=4, on_exit_callback=None):
        ...
        self._battle_tracker    = BattleTracker()
    ...
    def _load_assets(self):
        ...
        cuadro_h = line_h * 3 + borde_v * 2 + 40  # 3 líneas + tabs + bordes
    ...
    def _compute_layout(self):
        ...
        dot_gap = max(10, int(vida_w * 0.055)) + 6  # espacio para iconos debajo
    ...
    def handle_event(self, event):
        if event.type == pygame.VIDEORESIZE:
            ...
            self._compute_layout()
            self._poke_gifs = {} 
            if self._game_over_active and self.player_team:
                ci  = self.cuadro_inner
                f   = pkm_font(10)
                bh  = max(28, int(ci.height * 0.30))
                bw  = max(120, int(ci.width * 0.42))
                bx  = ci.right - bw - 4
                by  = ci.y + (ci.height - bh*2 - 8) // 2
                self._go_btns=[
                    Button(pygame.Rect(bx, by,      bw, bh), "Jugar de nuevo", f, tag="restart", text_align="left"),
                    Button(pygame.Rect(bx, by+bh+8, bw, bh), "Menu Principal", f, tag="menu",    text_align="left"),
                ]
    ...
    def _on_move(self, move_idx):
        ...
        if getattr(pp, 'taunted', False) and move.get("poder", 0) == 0:
            self._log_msg(f"😤 {pp.nombre} está provocado, no puede usar {move['nombre']}!", PKM_RED); return
        if self.ai_level == 4:
            ap = self.ai_team[self.ai_active_idx]
            self._battle_tracker.registrar_ataque(move, pp, ap)
        # Ida y Vuelta / Voltio Cambio: pedir destino ANTES pero cambiar DESPUÉS del daño
        if move["nombre"] in ["Ida y Vuelta","Voltio Cambio"]:
            avail = [i for i,p in enumerate(self.player_team)
                     if not p.fainted and i!=self.player_active_idx]
            if avail:
                self.buttons_locked = True
                def _elegido(new_idx):
                    self._dialog = None
                    self._ventana_cambio_abierta = False
                    ...
                    self.buttons_locked     = False
                    self._execute_turn(("move", move_idx))
                self._show_force_switch(
                    f"¿A quién cambiará {pp.nombre}?", avail, _elegido)
                return
        self._execute_turn(("move", move_idx))
    ...
    def _on_switch(self, target_idx):
        ...
        pp = self.player_team[self.player_active_idx]
        if pp.outrage_locked or (hasattr(pp,'flying_active') and pp.flying_active): return
        # Registrar cambio en memoria
        if self.ai_level == 4:
            ap = self.ai_team[self.ai_active_idx]
            nuevo = self.player_team[target_idx]
            self._battle_tracker.registrar_cambio(nuevo, ap)
    ...
    def _execute_turn(self, player_action):
        ...
        pp = self.player_team[self.player_active_idx]
        ap = self.ai_team[self.ai_active_idx]
        # Vuelo/Bote turno de aterrizaje → prioridad máxima
        # El orden se calcula ANTES del decremento, por eso turns==2 aquí = va a aterrizar
        pp_landing = getattr(pp,'flying_active',False) and getattr(pp,'flying_turns',0)==2
        ap_landing = getattr(ap,'flying_active',False) and getattr(ap,'flying_turns',0)==2
        if pp_landing and not ap_landing:
            orden = ["player","ai"]
        elif ap_landing and not pp_landing:
            orden = ["ai","player"]
        else:
    ...
    def _procesar_accion_siguiente(self, orden, index):
        ...
        if index == 0:
            pp = self.player_team[self.player_active_idx]
            ap = self.ai_team[self.ai_active_idx]
            # Solo decrementar si flying fue activado en un turno ANTERIOR (turns==2)
            # turns==2 → primer turno completo volando → decrementar a 1 (turno de aterrizar)
            if getattr(pp, 'flying_active', False) and getattr(pp, 'flying_turns', 0) == 2:
                pp.flying_turns   = 1
                pp.flying_charging = False  # turno de aterrizar y atacar
            if getattr(ap, 'flying_active', False) and getattr(ap, 'flying_turns', 0) == 2:
                ap.flying_turns   = 1
                ap.flying_charging = False
        quien = orden[index]
        ...
        if quien == "player":
            ...
            if getattr(p, 'flying_active', False):
                if getattr(p, 'flying_turns', 0) == 2:
                    pmv = None  # turno de carga: no actuar
                elif getattr(p, 'flying_turns', 0) == 1:
                    # turno de aterrizar: forzar movimiento Vuelo/Bote
            ...
            if getattr(p, 'flying_active', False):
                if getattr(p, 'flying_turns', 0) == 2:
                    amv = None  # turno de carga: no actuar
                elif getattr(p, 'flying_turns', 0) == 1:
                    flying_mv = getattr(p, 'flying_move', None)
                    if flying_mv:
                        for i, m in enumerate(p.movimientos):
    ...
    def _ejecutar_ataque_seq(self, atacante, defensor, move_idx, force_idx,
        ...
        if atacante.fainted or atacante.current_hp <= 0:
            if es_jugador and self.pending_switch:
                ...
                self.switch_source           = None
                self._player_force_idx = None
            # Siempre limpiar diálogo y flags al detectar muerte del atacante
            self._dialog                 = None
            self._ventana_cambio_abierta = False
            self._pending_log_lines      = []
            self._log_callback           = None
        ...
        if atacante.status == "freeze":
            if hasattr(atacante, 'freeze_turns'):
                ...
                if atacante.freeze_turns <= 0:
                    atacante.status = None
                    log_lines.append(f" ¡{atacante.nombre} se descongeló! No puede atacar este turno.")
                else:
                    log_lines.append(f" {atacante.nombre} no puede atacar porque está congelado!")
                self._log_lines_delayed(log_lines[:], callback); log_lines.clear(); return
            elif rand(0.2):
                atacante.status = None
                log_lines.append(f" ¡{atacante.nombre} se descongeló! No puede atacar este turno.")
        ...
        # Turno aterrizaje Vuelo/Bote: anunciar antes del golpe
        ...
        if move["pp"] <= 0:
            ...
            self._log_lines_delayed(log_lines[:], callback); log_lines.clear(); return
        # Turno carga de Vuelo/Bote:
        # - Primera vez (flying_active=False): activar estado y salir sin daño
        # - Ya activo en carga (flying_charging=True): salir sin daño
        if move["nombre"] in ("Vuelo", "Bote"):
            if not getattr(atacante, 'flying_active', False):
                ...
                # Turno de carga ya activo (no debería pasar normalmente)
                log_lines.append(f"🕊️ ¡{atacante.nombre} se elevó muy alto!")
                self._log_lines_delayed(log_lines[:], callback); log_lines.clear(); return
        ...
        if move["poder"] > 0:
            ...
            # Ida y Vuelta / Voltio Cambio: ejecutar cambio DESPUÉS del daño
            if es_jugador and self.pending_switch and self.pending_switch_idx is not None:
                new_idx  = self.pending_switch_idx
                old_name = atacante.nombre
                self.pending_switch     = False
                self.pending_switch_idx = None
                self.pending_move_idx   = None
                self.switch_source      = None
                new_p = self.player_team[new_idx]
                # 1. Mostrar el daño primero, luego hacer el cambio
                def _do_uturn_switch(ni=new_idx, on=old_name, np=new_p):
                    self.player_active_idx = ni
                    np.mods={"atk":0,"def":0,"spe":0,"evasion":0}
                    np.protect_success=True; np.protect_fail_count=0
                    h = apply_hazards_on_switch(np, self.player_hazards, True)
                    switch_lines = [f"🔄 ¡{on} regresó! ¡{np.nombre} salió al campo!"] + h
                    if np.fainted:
                        switch_lines.append(f"💀 ¡{np.nombre} fue derrotado por las trampas!")
                    # Actualizar barra del entrante y mostrar mensajes, luego continuar
                    self._sync_hp_on_switch()
                    self._trigger_hp_anim_from_state()
                    self._log_lines_delayed(switch_lines, callback)
                self._log_lines_delayed(log_lines[:], _do_uturn_switch)
                log_lines.clear()
                return
        else:
            apply_move_effects(atacante,defensor,move,effect_msgs,es_jugador,self.player_hazards,self.ai_hazards)
    ...
    def _aplicar_efectos_fin_turno_y_verificar(self):
        ...
        for pokemon in [pp, ap]:
            if getattr(pokemon,'flying_active',False) and getattr(pokemon,'flying_turns',0)==1:
                pokemon.flying_active   = False
                pokemon.flying_move     = None
                pokemon.flying_charging = False
                pokemon.flying_turns    = 0
        for pokemon in [pp, ap]:
            if getattr(pokemon, 'flying_active', False) and getattr(pokemon, 'flying_turns', 0) == 0:
                pokemon.flying_active = False
                pokemon.flying_move   = None
        for pokemon in [pp, ap]:
    ...
    def _iniciar_siguiente_turno(self):
        ...
        if outrage or flying:
            label = " sigue enfurecido" if outrage else "sigue volando"
            self._show_continue(
                f"{pp.nombre} {label}... (Presiona Continuar)",
                lambda: self._execute_turn(("move", -1))  
            )
        else:
            self._show_continue("Presiona Continuar para el siguiente turno",
                self._unlock_player
            )
    ...
    def _force_switch_player(self):
        ...
        def cb(idx):
            ...
            # Limpiar TODO estado pendiente que pueda rebloquear
            self._pending_log_lines      = []
            self._log_callback           = None
            self._fainted_wait_until     = 0
            self._procesando_derrotado   = False
            ...
            self._sync_hp_on_switch()
            self._trigger_hp_anim_from_state()
            # Desbloquear directamente sin pasar por _iniciar_siguiente_turno
            self.buttons_locked = False
            self.state          = self.STATE_MOVE_SELECT
            self._refresh_ui()
    ...
    def _game_over(self, winner):
        ...
        if self.ai_level == 4:
            jugador_gano = (winner == "player")
            actualizar_perfil(self._battle_tracker, jugador_gano)
    ...
    def _draw_vida_panels(self):
        ...
        self._draw_one_vida(self.rect_vida_player, self.vida_inner_player,
                            pp, self.player_team, self.player_active_idx,
                            "Jugador", self._hp_anim["player"]["cur"], flip=True)
        self._draw_one_vida(self.rect_vida_ai, self.vida_inner_ai,
                            ap, self.ai_team, self.ai_active_idx,
                            f"IA Nivel {self.ai_level}" + (" (MM)" if self.ai_level in (3,4) else ""), self._hp_anim["ai"]["cur"], flip=False)
    ...
    def _draw_one_vida(self, rect, inner, poke, team, active_idx, label, hp_cur, flip=False):
    ...
    def _draw_sprites(self):
        ...
        f_turn = pkm_font(13)
        turn_txt = f"TURNO {self.turn}"
        turn_surf = f_turn.render(turn_txt, True, PKM_BLACK)
        turn_x = self.W // 2 - turn_surf.get_width() // 2
        turn_y = self.rect_vida_player.y + (self.rect_vida_player.height - turn_surf.get_height()) // 2

        pad = 4
        bg_r = pygame.Rect(turn_x - pad, turn_y - pad,
                           turn_surf.get_width() + pad*2, turn_surf.get_height() + pad*2)
        pygame.draw.rect(self.screen, (240,240,255), bg_r, border_radius=4)
    ...
    def _draw_game_over_in_cuadro(self):
        ...
        for btn in self._go_btns:
            mouse = pygame.mouse.get_pos()
            btn.update_hover(mouse)
            pygame.draw.rect(self.screen, (230,230,255), btn.rect, border_radius=4)
            pygame.draw.rect(self.screen, (60,60,180),   btn.rect, 2, border_radius=4)
            col2 = SEL_COL if btn.hovered else PKM_BLACK
            cursor = "> " if btn.hovered else "  "
            rendered = f2.render(cursor+btn.text, True, col2)
            r = rendered.get_rect(midleft=(btn.rect.x+6, btn.rect.centery))
            self.screen.blit(rendered, r)
