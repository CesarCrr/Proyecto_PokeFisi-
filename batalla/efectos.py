...
def apply_move_effects(attacker, defender, move, log_lines, is_player_attacking, player_hazards, ai_hazards):
    ...
    # flying_charging=True cuando el pokemon está en turno de carga (interfaz)
    # en modo texto (logica_batalla) flying_turns==2 significa cargando
    defender_flying = (getattr(defender, 'flying_active', False) and
                       (getattr(defender, 'flying_charging', False) or
                        getattr(defender, 'flying_turns', 0) == 2))
    attacker_flying = (getattr(attacker, 'flying_active', False) and
                       (getattr(attacker, 'flying_charging', False) or
                        getattr(attacker, 'flying_turns', 0) == 2))
    ...
    if move_name == "Onda Trueno" and defender.status is None:
        if defender_flying:
            log_lines.append(f" ¡{defender.nombre} está volando y es inmune al estado!")
            return
        if rand(0.9):
    ...
    if "congelar" in effect and defender.status is None and not defender_flying and rand(0.1):
        ...
        log_lines.append(f" ¡{defender.nombre} fue congelado!")
    ...
    if move_name == "Polvo Veneno" and defender.status is None:
        ...
        log_lines.append(f" ¡{defender.nombre} fue envenenado por Polvo Veneno!")
        return

    if ("envenenar" in effect or effect == "envenena") and defender.status is None and not defender_flying and rand(0.3):
        defender.status = "poison"
        defender.poison_counter = 1
        log_lines.append(f" ¡{defender.nombre} fue envenenado!")
    ...
    if "veneno grave" in effect and defender.status is None:
        ...
        defender.poison_counter = 1
        log_lines.append(f" ¡{defender.nombre} fue gravemente envenenado!")
        return
    ...
    if move_name == "Amnesia":
        ...
        log_lines.append(f" ¡La Defensa de {attacker.nombre} subió mucho!")
        return
    ...
    if move_name == "Hipnosis" and defender.status is None:
        if defender_flying:
            log_lines.append(f" ¡{defender.nombre} está volando y es inmune a Hipnosis!")
            return
    ...
    if move_name == "Bostezo" and defender.status is None:
        if defender_flying:
            log_lines.append(f" ¡{defender.nombre} está volando y es inmune al Bostezo!")
            return
    ...
    if move_name == "Canto" and defender.status is None:
        if defender_flying:
            log_lines.append(f" ¡{defender.nombre} está volando y es inmune al Canto!")
            return
        defender.status = "sleep"
        defender.status_turns = random.randint(2, 3)
    ...
    if move_name == "Proteccion":
        ...
        if rand(prob):
            log_lines.append(f" {attacker.nombre} se protegió!")
    ...
    if move_name in ["Vuelo", "Bote"] and attacker.flying_turns == 0 and not getattr(attacker, 'flying_active', False):
        ...
        if move["pp"] > 0:
            move["pp"] -= 1
        log_lines.append(f" {attacker.nombre} comenzó a volar (turno 1). Atacará automáticamente el próximo turno.")
        return
    ...
    if move_name == "Puas Toxicas":
        if is_player_attacking:
            if ai_hazards["toxic_spikes"] < 2:
                ...
                log_lines.append(f" ¡{attacker.nombre} colocó una capa de púas tóxicas! (Nivel {ai_hazards['toxic_spikes']}/2)")
        else:
            if player_hazards["toxic_spikes"] < 2:
                player_hazards["toxic_spikes"] += 1
                log_lines.append(f" ¡{attacker.nombre} colocó una capa de púas tóxicas en tu campo! (Nivel {player_hazards['toxic_spikes']}/2)")
        return
